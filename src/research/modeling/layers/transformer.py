...
class L2TransformerDecoderLayer(nn.Module):
    def __init__(self, d_model: int, nhead: int, dim_feedforward: int,
                 dropout: float=0., activation: Optional[str]=None, tie_qk: bool=True):
    ...
    def _self_attn_block(self, x):
        x2 = self.norm_1(x)
        x2, _ = self.self_attn(x2, x2, x2) # no need for attention weights
        x = x + x2
        return x

    def _mha_block(self, x, mem):
        x2 = self.norm_2(x)
        x2, _ = self.mha(x2, mem, mem) # no need for attention weights
        x = x2 + x
        return x
    ...
    def forward(self, tgt, memory):
        tgt = self._self_attn_block(tgt)
        tgt = self._mha_block(tgt, memory)
        tgt = self._ffn(tgt)
        return tgt